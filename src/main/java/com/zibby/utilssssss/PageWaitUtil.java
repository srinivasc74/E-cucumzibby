package com.anthem.auto;

import com.anthem.auto.common.Configuration;
...
import static com.anthem.auto.PageUtil.highlightElement;
import static com.anthem.auto.WebDriverUtil.driver;
import static com.anthem.auto.WebDriverUtil.runWithTempTimeout;
import static com.anthem.auto.common.PrettyPrinter.prettyArray;
...
public final class PageWaitUtil {
    ...
    public static void waitForElements(final By by, int timeoutSeconds) {
        new FluentWait<WebDriver>(driver()).withTimeout(timeoutSeconds, TimeUnit.SECONDS)
                .pollingEvery(500, TimeUnit.MILLISECONDS).until(((Predicate<WebDriver>) (WebDriver d) -> {
                try {
                    return Boolean.valueOf(!d.findElements(by).isEmpty());
                } catch (Exception e) {
    ...
    /**
     * This is an efficient version of wait that allows the code to perform waits in tiers. It attempts
     * the action after every provided wait time until action returns true, or all of the waits are run.
     * It will only run as many waits as are required for action to return true.
     * <p>
     * Example Usage:
     * <code>
     ...
     * </code>
     * <p>
     * This looks ugly at first glance, but intellij will autogenerate most of it when you start typing new Callable.
     * I'm not sure if eclipse autogenerates, it didn't seem to from my initial look. +1 for IntelliJ
     * This will wait 100 milliseconds, then execute the method named call(). If that returns true, waitTiered will
     * return true. If not, waitTiered will repeat for 500, then 1000, then 2800 milliseconds.
     * <p>
     * Primary Uses/BenefitsPage:
     * There are some situations (such as waiting for animations) where we don't know how long it will take and
     * we don't have any events to work with that tell us when the action is completed. One solution is to add a
     * wait for a specific number of seconds. Let's say it will normally take between 100 and 5000 milliseconds for
     * the action we are waiting for to finish. With one wait, we would need to wait 5000 milliseconds every time
     * to ensure that the action has completed.
     * <p>
     * With waitTiered, we can break that 5 seconds into smaller waits to return faster when the website is
     * running faster and still wait longer if the website requires. This is far more efficient then just waiting.
     * Using the example above, let's say that it takes 200 milliseconds to run action.
     * Best Case (Success):
     * 100 ms wait + 200 ms action = 300 ms
     * <p>
     * Worst Case (Success or Failure):
     * 100 ms wait + 200 ms action +
     * 500 ms wait + 200 ms action +
     * 1000 ms wait + 200 ms action +
     * 2800 ms wait + 200 ms action = 5200 ms
     * <p>
     * Single Wait Best and Worst Case
     * 5000 ms wait + 200 ms action = 5200 ms
     * <p>
     * If you set up the wait times correctly it can run in the same amount of time as a normal wait in the worst
     * case, and better in every other case.
     *
     * @param action                  - An implementation of the Callable interface that must return a Boolean. Anonymous functions
     *                                work well for this.
     * @param waitTimesInMilliseconds - This is 1 to many wait times. If you don't provide a value this method will
     *                                throw an exception. The waits will be performed in the order they are passed
     ...
     * @throws Exception - If no wait times are provided, the wait fails, or any exception raised by action.
     */
